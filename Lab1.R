...
# categorical data, just like simtab did. Next, create a barplot of it, and a probability distribution bar plot. 

tab1 <- table(c(rep("A",25),rep("B",17),rep("C",98),rep("D",107)))
barplot(tab1)
...
# Perform 15000 simulations, and plot the results.

prob <- rbinom(15000, 15, 1/6)
probTab <- table(prob)
probTabp <- probTab/sum(probTab)
barplot(probTab)
barplot(probTabp)

# POISSON DISTRIBUTION
# rpois(t,lam) creates t random Poisson variables, each representing the number of successes occurring during a 
...
# ggplot will be what you create almost all your coursework plots in!
...
# From here on out, you'll need some extra libraries for the following types of plots.
# THESE ARE NOT EXPECTED IN YOUR COURSEWORK!
